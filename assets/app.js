...
function brokerCard(broker, index, maxLots) {
  ...
  const coreBody = `<div class="core-grid">
    ...
    <span><small>觀察期庫存</small><b>${lots(broker.inventory_lots)}</b></span>
    <span><small>觀察期成本</small><b>${price(broker.inventory_cost)}</b></span>
  ...
  <div class="capital-line"><span>觀察期淨投入</span><b>${money(broker.cumulative_net_amount)}</b><small>${broker.unrealized_pct == null ? "成本樣本不足" : `現價較成本 ${Number(broker.unrealized_pct) >= 0 ? "+" : ""}${price(broker.unrealized_pct)}%`}</small></div>`;
...
function render(data) {
  ...
  $("#signal-copy").textContent = data.signal?.reason || "正在累積 6820 的五日籌碼與價格樣本。";
...
function updateVisibleNet() {
  ...
  $("#visible-net-label").textContent = state.period === "core"
    ? "核心觀察期淨額" : state.side === "buy" ? "核心期間買超" : "核心期間賣超";
